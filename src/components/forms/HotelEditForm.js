    ...
    return ( 
        <form onSubmit={handleSubmit}>
          ...
          onChange: setLocation,
            ...
            {from && (
            <DatePicker defaultValue={moment(from, "YYYY-MM-DD")} placeholder="From date" className="form-control m-2" disabledDate={(current) => current && current.valueOf() < moment().subtract(1, 'days')} onChange={(date, dateString) => setValues({...values, from:dateString})} /> )}


            {to && (<DatePicker defaultValue={moment(to, "YYYY-MM-DD")} placeholder="To date" className="form-control m-2" disabledDate={(current) => current && current.valueOf() < moment().subtract(1, 'days')} onChange={(date, dateString) => setValues({...values, to:dateString})} />)}
